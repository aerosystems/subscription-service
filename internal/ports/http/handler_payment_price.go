...
func ModelToPriceResponse(prices map[entities.SubscriptionType]map[entities.SubscriptionDuration]int) *PriceResponse {
...
func (h Handler) GetPrices(c echo.Context) error {
	return c.JSON(http.StatusOK, ModelToPriceResponse(h.paymentUsecase.GetPrices(c.Request().Context())))
}
